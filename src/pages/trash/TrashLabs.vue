                            ...
                            <div class="card-body">
                                ...
                                >
                                    ...
                                    <template v-slot:cell(actions)="row">
                                        <b-button size="sm" @click="redirectInfo(row.item.id_lab)" class="mx-1 btn-table btn btn-sm btn-info" style="border: none;">Info</b-button>
                                        <b-button size="sm" @click="restore(row.item.id_lab)" class="mx-1 btn-table btn btn-sm btn-warning" style="border: none;">Restore</b-button>
                                        <b-button size="sm" @click="forceDelete(row.item.id_lab)" class="mx-1 btn-table btn btn-sm btn-danger" style="border: none;">Force Delete</b-button>
                                ...
                                <b-toast id="toast-restore" title="Row successfuly to restore." variant="success" toaster="b-toaster-bottom-right">
                                    Row will be restore to Labs table.
                                </b-toast>
...
import Info from '../../components/lab/Info'
import MultiInfo from '../../components/lab/MultiInfo'
...
export default {
    ...
    methods:{
        ...
        async redirectInfo(id){
            try{
                await this.$store.dispatch('lab/GET_INFO_BY_ID', id)
        ...
        async restore(id){
            try{
                await this.$store.dispatch('lab/RESTORE_ONE', id)
        ...
        async forceDelete(id){
            try{
                await this.$store.dispatch('lab/FORCEDELETE_ONE', id)
        ...
        async redirectMultiInfo(){
            try{
                await this.$store.commit('lab/SET_MULTI_INFO', this.selected)
        ...
        async multiRestore(){
            try{
                await this.$store.dispatch('lab/RESTORE_MULTI', this.selected)
        ...
        async multiForceDelete(){
            try{
                await this.$store.dispatch('lab/FORCEDELETE_MULTI', this.selected)
    ...
    async created(){
        await this.$store.dispatch('lab/GET_ALL_TRASH')
    }
}
</script>