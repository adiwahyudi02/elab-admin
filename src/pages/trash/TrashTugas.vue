                            ...
                            <div class="card-body">
                                ...
                                >
                                    ...
                                    <template v-slot:cell(actions)="row">
                                        <b-button size="sm" @click="redirectInfo(row.item.id_tugas)" class="mx-1 btn-table btn btn-sm btn-info" style="border: none;">Info</b-button>
                                        <b-button size="sm" @click="restore(row.item.id_tugas)" class="mx-1 btn-table btn btn-sm btn-warning" style="border: none;">Restore</b-button>
                                        <b-button size="sm" @click="forceDelete(row.item.id_tugas)" class="mx-1 btn-table btn btn-sm btn-danger" style="border: none;">Force Delete</b-button>
...
import Info from '../../components/tugas/Info'
import MultiInfo from '../../components/tugas/MultiInfo'
...
export default {
      ...
      return {
        fields: [
          ...
          { key: 'title', label: 'TItle', sortable: true},
            { key: 'nama_guru', label: 'Teacher', sortable: true},
            { key: 'nama_kelas', label: 'Class', sortable: true},
            { key: 'nama_lab', label: 'Lab', sortable: true},
            { key: 'nama_mapel', label: 'Subject', sortable: true},
    ...
    methods:{
        ...
        async redirectInfo(id){
            try{
                await this.$store.dispatch('tugas/GET_INFO_BY_ID', id)
        ...
        async restore(id){
            try{
                await this.$store.dispatch('tugas/RESTORE_ONE', id)
        ...
        async forceDelete(id){
            try{
                await this.$store.dispatch('tugas/FORCEDELETE_ONE', id)
        ...
        async redirectMultiInfo(){
            try{
                await this.$store.commit('tugas/SET_MULTI_INFO', this.selected)
        ...
        async multiRestore(){
            try{
                await this.$store.dispatch('tugas/RESTORE_MULTI', this.selected)
        ...
        async multiForceDelete(){
            try{
                await this.$store.dispatch('tugas/FORCEDELETE_MULTI', this.selected)
    ...
    async created(){
        await this.$store.dispatch('tugas/GET_ALL_TRASH')
    }
}
</script>