        ...
        <div class="card p-4">
          <div class="">
            <div class="card-body d-flex justify-content-start flex-wrap">
              <Create />
              <router-link :to="{ name: 'trash-admins' }" class="m-1"><b-btn v-b-modal.modal1 class="btn" style="border: none">Trash</b-btn></router-link>
            ...
            <div class="card-body">
              ...
              >
                ...
                <template v-slot:cell(actions)="row">
                  <b-button size="sm" @click="redirectInfo(row.item.id)" class="btn btn-table btn-info btn-sm mx-2" style="border: none;">Info</b-button>
                  <b-button size="sm" @click="softDelete(row.item.id)" class="btn btn-table btn-danger btn-sm mx-2" style="border: none;">Delete</b-button>
                  <!-- <b-button size="sm" @click="redirectEdit(row.item.id)" class="btn btn-table btn-warning btn-sm mx-2" style="border: none;">Edit</b-button> -->
                </template>
...
export default {
  ...
  methods: {
    ...
    async softDelete(id){
      try{
        await this.$store.dispatch('admin/SOFTDELETE_ONE', id)
    ...
    async multiSoftDelete(){
      try{
        await this.$store.dispatch('admin/SOFTDELETE_MULTI', this.selected)
        await this.$bvToast.show('toast-softdelete')
      }catch(err) {
        alert(err);
      }
  ...
  async created(){
    try {
      await this.$store.dispatch('admin/GET_ALL')
    } catch (error) {
      alert(error);
    }
  }
