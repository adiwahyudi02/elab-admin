        ...
        <div class="card p-4">
          <div class="">
            <div class="card-body d-flex justify-content-start flex-wrap">
              <Create />
              <router-link :to="{ name: 'trash-class' }" class="m-1"><b-btn v-b-modal.modal1 class="btn" style="border: none">Trash</b-btn></router-link>
            ...
            <div class="card-body">
              ...
              >
                ...
                <template v-slot:cell(actions)="row">
                  <b-button size="sm" @click="redirectInfo(row.item.id_kelas)" class="btn btn-table btn-info btn-sm mx-2" style="border: none;">Info</b-button>
                  <b-button size="sm" @click="softDelete(row.item.id_kelas)" class="btn btn-table btn-danger btn-sm mx-2" style="border: none;">Delete</b-button>
                  <b-button size="sm" @click="redirectEdit(row.item.id_kelas)" class="btn btn-table btn-warning btn-sm mx-2" style="border: none;">Edit</b-button>
                </template>
...
import Create from '../components/kelas/Create'
import Info from '../components/kelas/Info'
import Update from '../components/kelas/Update'
import MultiInfo from '../components/kelas/MultiInfo'
...
export default {
  data() {
    return {
      fields: [
          'index',
        { key: 'nama_kelas', label: 'Class name', sortable: true},
  ...
  methods: {
    ...
    async softDelete(id){
      try{
        await this.$store.dispatch('kelas/SOFTDELETE_ONE', id)
    ...
    async redirectInfo(id){
      try{
        await this.$store.dispatch('kelas/GET_INFO_BY_ID', id)
    ...
    async redirectMultiInfo(){
      try{
        await this.$store.commit('kelas/SET_MULTI_INFO', this.selected)
    ...
    async multiSoftDelete(){
      try{
        await this.$store.dispatch('kelas/SOFTDELETE_MULTI', this.selected)
        await this.$bvToast.show('toast-softdelete')
      }catch(err) {
        alert(err);
      }
  ...
  async created(){
    try {
      await this.$store.dispatch('kelas/GET_ALL')
    } catch (error) {
      alert(error);
    }
  }
