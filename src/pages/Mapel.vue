        ...
        <div class="card p-4">
          <div class="">
            <div class="card-body d-flex justify-content-start flex-wrap">
              <Create />
              <router-link :to="{ name: 'trash-subjects' }" class="m-1"><b-btn v-b-modal.modal1 class="btn" style="border: none">Trash</b-btn></router-link>
            ...
            <div class="card-body">
              ...
              >
                ...
                <template v-slot:cell(actions)="row">
                  <b-button size="sm" @click="redirectInfo(row.item.id_mapel)" class="btn btn-table btn-info btn-sm mx-2" style="border: none;">Info</b-button>
                  <b-button size="sm" @click="softDelete(row.item.id_mapel)" class="btn btn-table btn-danger btn-sm mx-2" style="border: none;">Delete</b-button>
                  <b-button size="sm" @click="redirectEdit(row.item.id_mapel)" class="btn btn-table btn-warning btn-sm mx-2" style="border: none;">Edit</b-button>
                </template>
...
import Create from '../components/mapel/Create'
import Info from '../components/mapel/Info'
import Update from '../components/mapel/Update'
import MultiInfo from '../components/mapel/MultiInfo'
...
export default {
  data() {
    return {
      fields: [
          'index',
        { key: 'nama_mapel', label: 'Subject name', sortable: true},
  ...
  methods: {
    ...
    async softDelete(id){
      try{
        await this.$store.dispatch('mapel/SOFTDELETE_ONE', id)
    ...
    async redirectInfo(id){
      try{
        await this.$store.dispatch('mapel/GET_INFO_BY_ID', id)
    ...
    async redirectMultiInfo(){
      try{
        await this.$store.commit('mapel/SET_MULTI_INFO', this.selected)
    ...
    async multiSoftDelete(){
      try{
        await this.$store.dispatch('mapel/SOFTDELETE_MULTI', this.selected)
        await this.$bvToast.show('toast-softdelete')
      }catch(err) {
        alert(err);
      }
  ...
  async created(){
    try {
      await this.$store.dispatch('mapel/GET_ALL')
    } catch (error) {
      alert(error);
    }
  }
