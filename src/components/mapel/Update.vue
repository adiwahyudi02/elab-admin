    ...
    <b-modal v-if="data != {}" id="modalUpdate" title="Edit subject" size="lg" :hide-footer="true">
        <b-form @submit.stop.prevent="update">
            <div class="form-group">
                ...
                <input type="text" v-model.trim="$v.form.nama_mapel.$model" class="form-control form-control-sm mb-0" id="lab">
                <div class="error" v-if="!$v.form.nama_mapel.required && $v.form.nama_mapel.$anyDirty">Subject name is required.</div>
            </div>
            <div>
...
export default {
    ...
    computed: {
        ...mapState({
            data (state) {
                const info = Object.assign({}, state.mapel.info)
                this.form = info
            },
    ...
    methods:{
        ...
        async update(){
            ...
            try{
                await this.$store.dispatch('mapel/UPDATE', this.form)
