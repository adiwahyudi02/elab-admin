  ...
  <div>
    <div class="card flex-fill w-100">
        <div class="card-header">
            <h4 class="text-success card-title mb-0"><b>Last week</b></h4>
        </div>
        <div class="card-body d-flex">
            <div class="align-self-center w-100">
                <div class="py-3">
                    <div class="chart chart-xs">
                        <canvas id="chartjs-dashboard-pie-this-week"></canvas>
                    </div>
                </div>
...
export default {
            ...
            return this.lab.map(item => {
                let a = this.data.filter(bill => {
                    ...
                    if ( bill.nama_lab == item.nama_lab) {
                        return data_billing >= new Date(d.setDate(d.getDate() - d.getDay()))
                    }
                })
    ...
    methods: {
        chartPie(){
            new Chart(document.getElementById("chartjs-dashboard-pie-this-week"), {
            type: 'pie',
            data: {
                labels: this.labels,
                datasets: [{
